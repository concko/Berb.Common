...
namespace LeagueSharp.Common
{
    ...
    public class LSOrbwalker
    {
        ...
        /// </summary>
        public static event EloBuddy.SDK.Orbwalker.UnkillableMinionHandler OnNonKillableMinion;
        ...
        private static void FireOnNonKillableMinion(Obj_AI_Base target, EloBuddy.SDK.Orbwalker.UnkillableMinionArgs args)
        {
            OnNonKillableMinion?.Invoke(target, args);
        }
        ...
        public static float GetRealAutoAttackRange(AttackableUnit target)
        {
            return ObjectManager.Player.GetAutoAttackRange(target);
        }
        ...
        public static bool InAutoAttackRange(AttackableUnit target)
        {
            return target.LSIsValidTarget(ObjectManager.Player.GetAutoAttackRange());
        }
        ...
        /// </summary>
        /// <returns>System.Single.</returns>
        public float GetMyProjectileSpeed()
        {
        ...
        public void AttackA(AttackableUnit target)
        {
            ...
            {
                LastAutoAttackCommandTick = Core.GameTickCount + Game.Ping + 100 - (int)(ObjectManager.Player.AttackCastDelay * 1000f);
        ...
        {
            if (Core.GameTickCount - LastMovementOrderTick < misc["delayMovement"].Cast<Slider>().CurrentValue)
            {
                return;
            }
            ...
            if (LimitAttackSpeed != 0)
            {
                ...
                if ((GameObjects.Player.AttackDelay < 1 / getDelay) && ((Player.LSCountEnemiesInRange(GetRealAutoAttackRange(Player)) >= 1) || (monster == null ? false : monster.Count() >= 1) || (minion == null ? false : minion.Count() >= 1)))
                {
                    return;
                }
            }
        ...
        public void Orbwalk(AttackableUnit target, Vector3 position, float extraWindup = 90, float holdAreaRadius = 0, bool useFixedDistance = true, bool randomizeMinDistance = true, bool meleePrediction = false)
        {
            ...
            if ((CanAttack() || LastAutoAttackCommandTick == 0) && Attack && !GameObjects.Player.IsCastingInterruptableSpell())
            {
                var gTarget = target;
                if (InAutoAttackRange(gTarget) && gTarget != null && !gTarget.IsDead && gTarget.IsVisible)
                {
        ...
        private void AfterAttack(AttackableUnit target)
        {
            ...
            LSEvents.FireAfterAttack(this, target);
        }
        ...
        public void OrbwalkerInit()
        {
            ...
            misc.Add("ExtraWindup", new Slider("Extra windup time", 0, 0, 200));
        ...
        {
            get { return getSliderItem(misc, "FarmDelay") + Game.Ping; }
        }
        ...
        public void SetOrbwalkerPoint(Vector3 point)
        {
                ...
                this.GetEnemyMinions()
                    .Any(
                        ...
                        Health.GetPrediction(
                            ...
                            this.FarmDelay,
                            HealthPredictionType.Simulated) < GameObjects.Player.CalculateDamageOnUnit(m, DamageType.Physical, GameObjects.Player.GetAutoAttackDamage(m)));
        }
        ...
        private List<Obj_AI_Minion> GetMinions(OrbwalkingMode mode)
        {
            ...
            foreach (var minion in EntityManager.MinionsAndMonsters.EnemyMinions.Where(m => IsValidUnit(m)))
            {
        ...
        public AttackableUnit GetTarget()
        {
            ...
            if (mode == OrbwalkingMode.LaneClear || mode == OrbwalkingMode.Mixed || mode == OrbwalkingMode.LastHit)
            {
                ...
                {
                    if (minion.IsHPBarRendered && minion.Health < GameObjects.Player.CalculateDamageOnUnit(minion, DamageType.Physical, GameObjects.Player.GetAutoAttackDamage(minion)))
                    {
                        return minion;
                    }
                    ...
                    else
                    {
                        ...
                        if (predHealth <= 0)
                        {
                            FireOnNonKillableMinion(minion, null);
                        }

                        if (predHealth > 0 && predHealth < GameObjects.Player.CalculateDamageOnUnit(minion, DamageType.Physical, GameObjects.Player.GetAutoAttackDamage(minion)))
                        {
                            return minion;
                        }
                    }
            ...
            // Forced Target
            if (_forcedTarget.LSIsValidTarget() && InAutoAttackRange(_forcedTarget) && _forcedTarget.IsVisible && _forcedTarget.IsHPBarRendered && !_forcedTarget.IsDead)
            {
            ...
            {
                foreach (var turret in GameObjects.EnemyTurrets.Where(t => t.LSIsValidTarget() && InAutoAttackRange(t)))
                {
                    return turret;
                }
            ...
            {
                var target = LSTargetSelector.GetTarget(-1f, DamageType.Physical);
                if (target.LSIsValidTarget() && InAutoAttackRange(target) && !target.IsDead && target.IsHPBarRendered && target.IsVisible)
                {
                    return target;
                }
            }
            ...
            {
                var minion = minions.FirstOrDefault(m => m.Team == GameObjectTeam.Neutral && m.IsHPBarRendered && m.IsVisible && !m.IsDead);
                if (minion != null)
                {
                    return minion;
                }
                            ...
                            return
                                (from minion in
                                 ...
                                 where
                                     ...
                                     > GameObjects.Player.CalculateDamageOnUnit(minion, DamageType.Physical, GameObjects.Player.GetAutoAttackDamage(minion))
                        ...
                        return (from minion in turretMinions.Where(x => !Health.HasMinionAggro(x))
                                let turret =
                                    GameObjects.AllyTurrets.FirstOrDefault(
                                        x => x.LSIsValidTarget(950f, false, minion.Position))
                                where
                                    turret != null
                                    && (int)minion.Health % (int)turret.GetAutoAttackDamage(minion)
                                    > GameObjects.Player.CalculateDamageOnUnit(minion, DamageType.Physical, GameObjects.Player.GetAutoAttackDamage(minion))
            ...
            if (mode == OrbwalkingMode.LaneClear)
            {
                if (!this.ShouldWait())
                {
                    if (this.LaneClearMinion.LSIsValidTarget() && InAutoAttackRange(this.LaneClearMinion) && !this.LaneClearMinion.IsDead && this.LaneClearMinion.IsHPBarRendered && this.LaneClearMinion.IsVisible)
                    {
                        ...
                        var predHealth = Health.GetPrediction(this.LaneClearMinion, (int)(GameObjects.Player.AttackDelay * 1000 * LaneClearWaitTime), this.FarmDelay, HealthPredictionType.Simulated);
                        if (predHealth >= 2 * GameObjects.Player.GetAutoAttackDamage(this.LaneClearMinion) || Math.Abs(predHealth - this.LaneClearMinion.Health) < float.Epsilon)
                        {
                            return this.LaneClearMinion;
                        }
                    }
                    ...
                    foreach (var minion in minions.Where(m => m.Team != GameObjectTeam.Neutral && !m.IsDead && m.IsHPBarRendered && m.IsVisible))
                    {
                        ...
                        var predHealth = Health.GetPrediction(minion, (int)(GameObjects.Player.AttackDelay * 1000 * LaneClearWaitTime), this.FarmDelay, HealthPredictionType.Simulated);
            ...
            {
                if (minions.Any() && !GameObjects.EnemyHeroes.Any(e => e.LSIsValidTarget(e.GetRealAutoAttackRange() * 2f) && !e.IsDead && e.IsHPBarRendered && e.IsVisible))
                {
        ...
        public static float getWindUp()
        {
            if (Game.Ping <= 30)
            {
                return getSliderItem(misc, "ExtraWindup") + (Game.Ping * 3.4f);
            }
            else
            {
                return getSliderItem(misc, "ExtraWindup") + (Game.Ping * 1.3f);
            }
        }
